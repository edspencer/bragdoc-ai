...
export function StandupUpdateSection({ standupId, selectedAchievements, standupInstructions }: StandupUpdateSectionProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/standups/${standupId}/achievements-summary`, {
        method: "POST",
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to generate standup update')
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to save standup update')
    } finally {
      setIsSaving(false)
    }
  }
          ...
          <div className="flex gap-2">
            {isEditing ? (
              ...
              <>
                ...
                <Button size="sm" onClick={handleGenerate} disabled={isGenerating || selectedAchievements.length === 0}>
                  <IconRefresh className={`size-4 mr-2 ${isGenerating ? "animate-spin" : ""}`} />
      ...
      <CardContent>
        ...
        {selectedAchievements.length === 0 && (
          <p className="text-sm text-muted-foreground mt-2">
            Select one or more achievements from the Recent Achievements table to generate your standup update.
          </p>
        )}
      </CardContent>
    </Card>
