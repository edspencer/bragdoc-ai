...
export function StandupForm({ onSuccess, onCancel, initialData, isEdit = false }: StandupFormProps) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      toast.success(isEdit ? "Standup updated successfully" : "Standup created successfully")
      onSuccess()
    } catch (error) {
      console.error('Error saving standup:', error)
      toast.error(error instanceof Error ? error.message : 'Failed to save standup')
    } finally {
          ...
          <div className="flex gap-2">
            {DAYS.map((day, index) => (
              <div key={index} className="flex flex-col items-center gap-1">
                <Checkbox checked={isDaySelected(day.value)} onCheckedChange={() => toggleDay(day.value)} />
                <span className="text-xs text-muted-foreground">{day.label}</span>
              </div>
            ))}
          </div>
          {daysMask === 0 && <p className="text-sm text-destructive">Please select at least one day</p>}
        </div>
        ...
        <Collapsible open={isAdvancedOpen} onOpenChange={setIsAdvancedOpen}>
          <CollapsibleTrigger asChild>
            <Button variant="ghost" className="flex items-center gap-2 p-0 h-auto">
              <span>Advanced</span>
              <IconChevronDown className={`h-4 w-4 transition-transform ${isAdvancedOpen ? "rotate-180" : ""}`} />
            </Button>
          </CollapsibleTrigger>
