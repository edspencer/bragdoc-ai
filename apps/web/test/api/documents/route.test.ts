...
describe('Documents API', () => {
  ...
  beforeEach(async () => {
    ...
    await db.delete(document).where(eq(document.userId, mockUser.id)).execute();
    await db.delete(project).where(eq(project.userId, mockUser.id)).execute();
    await db.delete(company).where(eq(company.userId, mockUser.id)).execute();
  ...
  afterEach(async () => {
    ...
    await db.delete(project).where(eq(project.userId, mockUser.id)).execute();
    await db.delete(company).where(eq(company.userId, mockUser.id)).execute();
    await db.delete(user).where(eq(user.id, mockUser.id)).execute();
  });
  ...
  describe('DELETE /api/documents/[id]', () => {
    it('deletes a document', async () => {
      ...
      const url = new URL(`http://localhost/api/documents/${testDoc.id}`);
      const response = await DELETE(new NextRequest(url), {
        params: Promise.resolve({ id: testDoc.id }),
      });
  ...
  describe('Document Sharing', () => {
    it('generates a share token', async () => {
      ...
      const response = await shareDocument(new NextRequest(url), {
        params: Promise.resolve({ id: testDoc.id }),
      });
    ...
    it('revokes a share token', async () => {
      ...
      const url = new URL(`http://localhost/api/documents/${testDoc.id}/share`);
      const response = await unshareDocument(new NextRequest(url), {
        params: Promise.resolve({ id: testDoc.id }),
      });
