...
describe('CommitCache', () => {
    const mockFs = {
        ...
        unlink: jest.mocked(promises_1.unlink),
        readdir: jest.mocked(promises_1.readdir),
        mkdir: jest.mocked(promises_1.mkdir),
        chmod: jest.mocked(promises_1.chmod),
        appendFile: jest.mocked(promises_1.appendFile),
    };
    ...
    beforeEach(() => {
        ...
        cache = new commits_1.CommitCache();
    ...
    describe('add', () => {
        it('should add new commit hashes to cache file', async () => {
            ...
            const cachePath = (0, node_path_1.join)(TEST_CACHE_DIR, `${repoName.replace(/[^a-zA-Z0-9-]/g, '_')}.txt`);
    ...
    describe('clear', () => {
        ...
        it('should clear all repository caches', async () => {
            ...
            for (const file of files) {
                expect(mockFs.unlink).toHaveBeenCalledWith((0, node_path_1.join)(TEST_CACHE_DIR, file.name));
            }
        });
...
function createMockDirent(name, isFile = true) {
    return {
        name,
        isFile: () => isFile,
        isDirectory: () => !isFile,
