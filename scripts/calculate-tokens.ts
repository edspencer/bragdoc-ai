...
function getAllFiles(dirPath: string, arrayOfFiles: string[] = []): string[] {
  ...
  files.forEach((file) => {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      // biome-ignore lint/style/noParameterAssign: We intentionally mutate the parameter here
...
async function main() {
  ...
  console.log(
    `Approximate cost to process (at $0.01 per 1K tokens): $${((totalTokens / 1000) * 0.01).toFixed(2)}`,
  );
