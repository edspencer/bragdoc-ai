...
const getPageName = (url: string) => {
  ...
  return pathname === '/' ? 'marketing' : pathname.split('/').filter(Boolean).join('-') || 'marketing';
};
...
(async () => {
  ...
  try {
    ...
    const pageName = getPageName(url);
    console.log(`Screenshots captured in temp/${pageName}-light.png and temp/${pageName}-dark.png`);
  } catch (error) {
