...
**Purpose**: Stores semantic clusters of related achievements discovered by DBSCAN clustering.

**Key Fields**:
- `centroidEmbedding`: Cached mean embedding for fast similarity matching (used in incremental assignment)
- `achievementCount`: Denormalized count for quick UI rendering without aggregation queries
- `isArchived`: Soft deletion allows historical tracking without data loss
...
**Purpose**: Tracks clustering history and decisions (used to decide when to re-cluster).

**Key Fields**:
- `lastFullClusteringAt`: Used to determine if >30 days have passed
- `achievementCountAtLastClustering`: Used to calculate if +10% or +50 achievements added
- `epsilon`, `minPts`: Parameters used for reproducibility and debugging

#### Achievement Table - New Fields

The existing `achievement` table was enhanced with embedding and workstream assignment fields:

```typescript
// Added to achievement table:
workstreamId: uuid('workstream_id').references(() => workstream.id, {
  onDelete: 'set null' // Allows deletion of workstreams without cascading
}),
workstreamSource: varchar('workstream_source', { length: 16 }), // 'ai' | 'user'
...
**Purpose**: Enables semantic grouping and tracks how achievements are assigned.

**Key Fields**:
- `embedding`: OpenAI text-embedding-3-small (1536 dimensions)
- `workstreamSource`: Distinguishes AI assignments from user overrides
  - `'ai'`: Can be changed during re-clustering
  - `'user'`: Preserved during re-clustering

### pgvector Extension

**Requirement**: PostgreSQL must have pgvector extension installed.
...
**Cloud Providers with pgvector Support**:
- **Neon**: Built-in support, enabled by default
- **Vercel Postgres**: Available with pgvector extension
- **AWS RDS**: Available with pgvector extension
- **Local Development**: Install via package manager (macOS: `brew install pgvector`)

### Query Patterns for Workstreams

#### Fetch Workstreams by User

```typescript
export async function getWorkstreamsByUserId(userId: string, includeArchived = false) {
  const query = db
    .select()
    .from(workstream)
  ...
  if (!includeArchived) {
    query.where(eq(workstream.isArchived, false));
  }

  return query.orderBy(desc(workstream.achievementCount));
}
```
...
```typescript
export async function getAchievementsWithoutEmbeddings(userId: string) {
  return db
    .select()
    .from(achievement)
    .where(
...
#### Incremental Assignment - Find Nearest Workstream

```typescript
// Find best matching workstream using vector similarity
// Note: Drizzle doesn't have built-in vector distance operators
// Use raw SQL or fetch workstreams and calculate in application layer

export async function findNearestWorkstream(
  achievementEmbedding: number[],
  workstreamCentroids: Array<{ id: string; embedding: number[] }>
) {
  // Calculate cosine distance in application layer
  let minDistance = Infinity;
  let nearestWorkstreamId: string | null = null;

  for (const ws of workstreamCentroids) {
    const distance = cosineDistance(achievementEmbedding, ws.embedding);
    if (distance < minDistance) {
      minDistance = distance;
      nearestWorkstreamId = ws.id;
    }
  }

  return { workstreamId: nearestWorkstreamId, confidence: 1 - minDistance };
}
```

#### Update Workstream Centroid

```typescript
export async function updateWorkstreamCentroid(workstreamId: string) {
  // Fetch all achievements in workstream
  const achievements = await db
    .select()
    .from(achievement)
  ...
  // Update workstream with new centroid
  await db
    .update(workstream)
    .set({
...
CREATE INDEX idx_workstream_user_id ON "Workstream"("user_id");
CREATE INDEX idx_workstream_metadata_user_id ON "WorkstreamMetadata"("user_id");
CREATE INDEX idx_achievement_workstream_id ON "Achievement"("workstream_id");
CREATE INDEX idx_achievement_embedding_generated_at ON "Achievement"("embedding_generated_at");

-- Vector index for approximate nearest neighbor search (optional, for scaling)
-- Requires pgvector extension with ivfflat index type
-- CREATE INDEX idx_achievement_embedding_ivfflat ON "Achievement"
--   USING ivfflat (embedding vector_cosine_ops);
```

#### Query Optimization Tips

1. **Centroid Caching**: Always use cached `centroidEmbedding` instead of recalculating
2. **Achievement Count Denormalization**: Use `achievementCount` on workstream instead of COUNT aggregation
3. **Batch Operations**: Use `db.batch()` for multiple workstream updates
4. **Vector Operations**: For large-scale similarity search, consider pgvector's IVFFLAT index

#### Scaling Beyond 1000 Achievements

For users with very large achievement datasets:

1. **Consider Background Job Processing**: Move clustering to async job queue
2. **Implement Pagination**: Load achievements in batches during clustering
3. **Use Database Cursors**: Stream large result sets to avoid memory issues
4. **Archive Old Workstreams**: Soft delete workstreams older than certain date

---

**Last Updated**: 2025-11-06 (Workstreams feature documentation)
