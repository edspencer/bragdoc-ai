...
const projectMatchResponseSchema = z.object({
  projectId: z.string().nullable().describe('ID of the matching project, or null if no match found'),
  confidence: z.number().min(0).max(1).describe('Confidence score between 0 and 1'),
  reasoning: z.string().describe('Explanation of why this project matches or why no match was found'),
});
...
- reasoning: Explain why you think it's a match or why no match was found`,
      ...
      {
        role: 'user',
        content: JSON.stringify(projects.map(p => ({
          id: p.id,
          name: p.name,
